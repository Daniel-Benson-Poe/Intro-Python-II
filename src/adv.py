...
to north. The smell of gold permeates the air.""",
                items_useable=[items['key'].name, items['torch'].name]),
...
def input_parser():
    ...
    command = input("'n' (go north), 's' (go south), 'e' (go east), 'w' (go west), 'p' (pickup item), 'u' (use item), or 'q' (quit game)\n")
...
while True:
    ...
    if command == 'u':
        ...
        if player.inventory:
            ...
            selected_item = input([f"{i.name}, {i.description}" for i in player.inventory])
            # print(selected_item) testing purposes
            if selected_item.capitalize() in player.current_room.items_useable:
